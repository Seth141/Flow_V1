...
function KanbanBoard() {
  ...
  const [columns, setColumns] = useState(['Backlog', 'Sprint 2', 'Doing', 'Completed']);
  ...
  const onDragEnd = (result) => {
    ...
    const [movedTask] = updatedTasks.splice(updatedTasks.findIndex(task => task.id.toString() === draggableId), 1);
    movedTask.status = destination.droppableId;

    const destinationTasks = updatedTasks.filter(task => task.status === destination.droppableId);
    destinationTasks.splice(destination.index, 0, movedTask);

    const newTasks = updatedTasks.filter(task => task.status !== destination.droppableId).concat(destinationTasks);
  ...
  const handleColumnTitleChange = (oldTitle, newTitle) => {
    const updatedColumns = columns.map(col => col === oldTitle ? newTitle : col);
          ...
          {columns.map(column => (
            <div key={column} className="kanban-column">
              <div className="column-header">
                <EditableColumnTitle 
                  title={column} 
                  onTitleChange={(newTitle) => handleColumnTitleChange(column, newTitle)} 
                />
              </div>
                      ...
                      .map((task, index) => (
                        <Draggable key={task.id} draggableId={task.id.toString()} index={index}>
                          {(provided) => (
                            ...
                            >
                              <EditableTaskCard task={task} onTaskUpdate={handleTaskUpdate} />
                            </div>
                          )}
                        </Draggable>
                      ))
